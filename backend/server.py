...
restore_token_exp_minutes = config.restore_token_exp_minutes
access_secret = config.access_secret
refresh_secret = config.refresh_secret
restore_secret = config.restore_secret
...
engine = create_engine(f"postgresql://{config.db_user}:{config.db_passw}@{config.db_host}:{config.db_port}/{config.db_name}")
...
def send_email(login, password, sender, recipient, subject, body):
    smtp_server = "mail.privateemail.com"
    smtp_port = 465
    smtp_user = login # Replace with your Namecheap email address
    smtp_password = password # Replace with your Namecheap email password
